...
def extract_datetime(line: str) -> datetime:
    ...
    try:
        ...
        second = 0
        line_id = extract_line_identifier(line)
        logger.warning('second "%s" was converted into %d, %s', line[13:15], second, line_id)
    except Exception as e:
        raise e
    return datetime(year, month, day, hour, minute, second)


def convert_latitude_longitude_deg(line: str, degree: int, minute: int, second: int) -> float:
    ...
    try:
        ...
        deg_second = 0.0
        line_id = extract_line_identifier(line)
        logger.warning('second "%s" was converted into %d, %s', _second, deg_second, line_id)
    except Exception as e:
        raise e
...
def extract_depth(line: str) -> float:
    ...
    try:
        ...
        _decimal = 0
        line_id = extract_line_identifier(line)
        logger.warning('decimal "%s" was converted into %d, %s', line[47:49], _decimal, line_id)
    except Exception as e:
        raise e
